...
public class MyBounderAdapter extends RecyclerView.Adapter<BindingViewHolder<ListitemBounderBinding>> {
    private ArrayList<MyBounderData> myBounderDatas = new ArrayList<>();
    ...
    public void add(ArrayList<MyBounderData> datas, Context context, Activity activity, Fragment fragment) {
        mContext = context;
        mActivity= activity;
        this.myBounderDatas = datas;
    ...
    @Override
    public BindingViewHolder<ListitemBounderBinding> onCreateViewHolder(ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());

        return new BindingViewHolder<>(inflater.inflate(R.layout.listitem_bounder, parent, false));
    }

    @Override
    public void onBindViewHolder(final BindingViewHolder<ListitemBounderBinding> holder, final int position) {
        final MyBounderData data = myBounderDatas.get(position);
        ListitemBounderBinding binding = holder.binding();
        ...
        userService.CANCEL_FRIEND_API_CALL(AppController.getInstance().getPrefManager().getUser().getUser_id(),friend_id).enqueue(new Callback<CancelFriendAPI>() {
            ...
            public void onResponse(Call<CancelFriendAPI> call, Response<CancelFriendAPI> response) {
                if(response.body().success){
                    myBounderDatas.remove(position);
                    notifyDataSetChanged();
                }else{
